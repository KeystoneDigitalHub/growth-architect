...
const AuditForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (budget < 500) {
      setThankYouMsg("Focusing on AI Automation to maximize your current efficiency. Start small, scale smart.");
    } else if (form.website_url) {
      setThankYouMsg("Check your email for your Free Visual Conversion Audit — we'll highlight structural improvements to increase trust and conversions.");
    } else {
      setThankYouMsg("Your audit request is confirmed. Our team will analyze your brand positioning and deliver a strategic growth roadmap.");
    }
    ...
    return (
      <section id="audit" className="section-padding">
        <AnimatedSection className="max-w-xl mx-auto text-center">
          ...
          <p className="text-xs text-muted-foreground">Keystone Growth Hub — Diagnosis → Strategy → Execution</p>
        </AnimatedSection>
      </section>
    );
  }
  ...
  return (
    <section id="audit" className="section-padding">
      <AnimatedSection className="max-w-xl mx-auto">
        ...
        <h2 className="text-3xl sm:text-4xl font-bold mb-4 tracking-tight">Ready for Predictable Growth?</h2>
        <p className="text-muted-foreground mb-8">Tell us about your business. We'll diagnose the friction and map a growth path.</p>
